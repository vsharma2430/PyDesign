...
class member_group:
    def __init__(self,Staad_objects:OpenSTAAD_objects, id=None, members=None, profiles=None, preference=None):
        ...
        self.preference = preference if preference is not None else []
    ...
    def set_members_property(self,index):
        if(index > -1 and index < len(self.profiles)):
            for member in self.members:
                ...
                print(f'Assignment to {member} of {self.profiles[index]} : {result}')
                
    def get_utilization_ratios(self):
        self.results = get_member_steel_design_results(self.output,self.members)
